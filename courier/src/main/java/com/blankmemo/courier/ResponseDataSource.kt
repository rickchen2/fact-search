...
import com.blankmemo.courier.coroutines.SuspensionFunction
import com.blankmemo.courier.disposables.CompositeDisposable
import com.blankmemo.courier.disposables.disposable
import com.blankmemo.courier.executors.ArchTaskExecutor
...
class ResponseDataSource<T> : DataSource<T> {
    ...
    var concatStrategy: DataSource.ConcatStrategy = DataSource.ConcatStrategy.CONTINUOUS
    ...
    fun dataRetainPolicy(dataRetainPolicy: DataRetainPolicy): ResponseDataSource<T> = apply {
        this.dataRetainPolicy = dataRetainPolicy
    }

    /** combine a call and callback instances for caching data. */
    override fun combine(call: Call<T>, callback: Callback<T>?): ResponseDataSource<T> =
        apply {
            this.call = call
    ...
    override fun retry(retryCount: Int, interval: Long): ResponseDataSource<T> = apply {
    ...
    override fun request(): ResponseDataSource<T> = apply {
    ...
    override fun joinDisposable(disposable: CompositeDisposable): ResponseDataSource<T> =
        apply {
    ...
    override fun observeResponse(observer: ResponseObserver<T>): ResponseDataSource<T> =
        apply {
